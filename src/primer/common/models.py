...
class DeviceToken(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    token_hash: Mapped[str] = mapped_column(String(255), nullable=False, unique=True)
    token_last_four: Mapped[str] = mapped_column(String(8), nullable=False)
    revoked: Mapped[bool] = mapped_column(Boolean, default=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
...
class Intervention(Base):
    ...
    experiment_hypothesis: Mapped[str | None] = mapped_column(Text, nullable=True)
    experiment_target_cohort: Mapped[str | None] = mapped_column(String(100), nullable=True)
    experiment_success_criteria: Mapped[str | None] = mapped_column(Text, nullable=True)
