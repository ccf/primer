...
def upsert_session(db: Session, engineer_id: str, payload: SessionIngestPayload) -> bool:
    ...
    for field in [
        "project_path", "project_name", "git_branch", "claude_version",
        "permission_mode", "end_reason", "started_at", "ended_at",
        "duration_seconds", "message_count", "user_message_count",
        "assistant_message_count", "tool_call_count", "input_tokens",
    ...
    if payload.tool_usages:
        ...
        for tu in payload.tool_usages:
            db.add(ToolUsage(session_id=session.id, tool_name=tu.tool_name, call_count=tu.call_count))
...
def upsert_facets(db: Session, session_id: str, facets: SessionFacetsPayload) -> None:
    ...
    for field in [
        ...
        "primary_success", "claude_helpfulness", "brief_summary",
