...
def get_recommendations(db: Session, team_id: str | None = None) -> list[Recommendation]:
    ...
    if overview.avg_session_duration and overview.total_sessions > 5:
        avg_tokens = (overview.total_input_tokens + overview.total_output_tokens) / overview.total_sessions
    ...
    if tools and tools[0].total_calls > 0:
        ...
        if top_tool_share > 0.6:
            recs.append(Recommendation(
                ...
                description=(
                    f"{tools[0].tool_name} accounts for {top_tool_share:.0%} of all tool calls. "
